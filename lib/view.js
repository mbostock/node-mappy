...
var _view = module.exports = function() {
  ...
  view.point = function(coordinate) {
    var kc = Math.pow(2, center[2] - coordinate[2]),
  ...
  view.panBy = function(x) {
    var kt = Math.pow(2, center[2]),
        dx = x[0] * kt,
        dy = x[1] * kt;
    return view.center([
      center[0] + (angleSini * dy - angleCosi * dx) / coordinateSize[0],
      center[1] + (angleSini * dx + angleCosi * dy) / coordinateSize[1],
      center[2]
    ]);
  };
  ...
  view.zoomBy = function(x, point, coordinate) {
    if (arguments.length < 2) return view.center([center[0], center[1], center[2] + x]);

    // compute the coordinate of the center point
    if (arguments.length < 3) coordinate = view.coordinate(point);

    // update the zoom level
    view.center([center[0], center[1], center[2] + x]);

    // compute the new point of the coordinate
    var point2 = view.point(coordinate);
