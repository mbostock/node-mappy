...
var Canvas = require("canvas"), // ../../node-canvas/lib/canvas"),
    Image = Canvas.Image,
...
module.exports = function() {
  ...
  image.render = function(context, callback) {
    ...
    var dz = viewZoom - (viewZoom = Math.round(viewZoom)) + (zoom ? viewZoom - zoom(viewZoom) : 0),
        kz = Math.pow(2, -dz);
    ...
    var offcanvas = new Canvas((x1 - x0) * dx, (y1 - y0) * dy),
    ...
    tiles.forEach(function(tile) {
      _cache(url(tile), function(buffer) {
        ...
        if (!--remaining) {
          image.src = offcanvas.toBuffer();

          context.save();
          context.translate(viewSize[0] / 2, viewSize[1] / 2);
          context.rotate(viewAngle);
          context.scale(1 / kz, 1 / kz);
          context.drawImage(image, dx * (x0 - viewCenter[0] * kz), dy * (y0 - viewCenter[1] * kz));
          context.restore();
